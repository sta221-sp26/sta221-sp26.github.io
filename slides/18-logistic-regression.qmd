...
Let's consider our usual linear model $\hat{\mathbf{y}} = \mathbf{X}\hat{\boldsymbol{\beta}}$
...
Instead of modeling $\pi_i$ directly, we will model $\text{logit}(\pi_i) = \text{logit}\big(\frac{\pi_i}{1 - \pi_i}\big)$ such that

$$
\text{logit}(\pi_i) = \text{logit}\Big(\frac{\pi_i}{1 - \pi_i}\Big) = \mathbf{x}_i^\mathsf{T}\boldsymbol{\beta}
$$
...
$$
\text{logit}(\pi_i) = \text{logit}\Big(\frac{\pi_i}{1 - \pi_i}\Big) $$
...
## From logit to probabilities {.incremental}
...
The likelihood is the joint density of $y_i, \ldots, y_n$ as a function of the unknown $\boldsymbol{\beta}$
...
$$
\begin{aligned}
\log L(\boldsymbol\beta | \mathbf{X}, \mathbf{y}) &= \sum_{i=1}^n\log(\pi_i^{y_i}(1-\pi_i)^{1-y_i})\\[8pt]
...
$$
\begin{aligned}
\log L(\boldsymbol{\beta} \mid \mathbf{X}, \mathbf{y}) &= \sum_{i=1}^n y_i \log\left(\frac{\exp\{\mathbf{x}_i^\mathsf{T}\boldsymbol{\beta}\}}{1 + \exp\{\mathbf{x}_i^\mathsf{T}\boldsymbol{\beta}\}}\right)+ (1 - y_i)\log\left(\frac{1}{1 + \exp\{\mathbf{x}_i^\mathsf{T}\boldsymbol{\beta}\}}\right) \\[8pt]
