...
## Positive (semi-)definite matrices {background-color="#ccddeb"}
...
::: incremental
-   For any $\mathbf{z} = \mathbf{X}\boldsymbol{\beta}$ in $\text{Col}(\mathbf{X})$, the vector $\mathbf{e} = \mathbf{y} - \mathbf{Xb}$ is the difference between $\mathbf{y}$ and $\mathbf{X}\boldsymbol{\beta}$.
