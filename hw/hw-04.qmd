...
a.  Show that the Hessian for the log-likelihood of logistic regression, $\frac{\partial^2}{\partial \boldsymbol{\beta}\partial\boldsymbol{\beta}^\mathsf{T}}\log L$ , can be written as
...
b.  The Hessian in part (a) describes the curvature of the log-likelihood function (i.e., larger magnitude of the Hessian corresponds to a steeper peak). Describe how the curvature of log-likelihood is related to $Var(\hat{\boldsymbol{\beta}})$, the variance of the estimated coefficients.
...
Produce the ROC curve for the model from the previous exercise and compute the area under curve (AUC). Write 1 - 2 sentences evaluating the model performance.
